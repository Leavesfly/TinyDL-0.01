...
public class CosDataSet extends ArrayDataset {
    public CosDataSet(int batchSize) {
        super(batchSize);
    }

    @Override
    protected DataSet build(int batchSize, NdArray[] _xs, NdArray[] _ys) {
        ArrayDataset dataSet = new CosDataSet(batchSize);
        dataSet.setXs(_xs);
        dataSet.setYs(_ys);
        return dataSet;
    }
    ...
    public void doPrepare() {
        int size = 1000;
        int tmpSize = size + 1;
        NdArray tmp = NdArray.linSpace(0f, (float) (Math.PI * 2), tmpSize);

        //训练数据构造
        NdArray[] tmpArray = new NdArray[tmpSize];
        for (int i = 0; i < tmpSize; i++) {
            tmpArray[i] = new NdArray(Math.sin(tmp.getMatrix()[0][i]));
        }
        ...
        for (int i = 0; i < tmpSize; i++) {
            tmpArray[i] = new NdArray(Math.cos(tmp.getMatrix()[0][i]));
        }
