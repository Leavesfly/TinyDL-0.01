...
`MlpBlock`实现多层感知机，支持任意数量的隐藏层。输入层大小为2（二维坐标），两个隐藏层均为30个神经元，输出层为3（三分类）。默认使用Sigmoid激活函数，可通过配置切换为ReLU。
...
系统采用`SoftmaxCrossEntropy`作为损失函数，结合`SGD`优化器进行参数更新。`Trainer`封装训练流程，支持自动训练模式，包含监控、评估与绘图功能。
...
建议采用学习率衰减策略，在训练后期降低学习率以提高收敛精度。可通过观察`Plot`生成的损失曲线判断是否收敛。
